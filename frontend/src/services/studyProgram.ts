...
export const callUpdateStudyProgram = async (
  id: number,
  studyProgram: IProgram
) => {
  return await axiosInstance.put(`/StudyProgram/${id}`, studyProgram);
};

export const callDeleteStudyProgram = async (id: number) => {
  return (await axiosInstance.delete(`/StudyProgram/${id}`)).data;
};
